...
class AppPixivAPI(BasePixivAPI):
    ...
    async def set_api_proxy(self, proxy_hosts="http://app-api.pixivlite.com"):
        """Set proxy hosts: eg pixivlite.com"""
        self.hosts = proxy_hosts

    # Check auth and set BearerToken to headers
    async def no_auth_requests_call(self, method, url, headers=None, params=None, data=None, req_auth=True):
        default_headers = {
            'App-OS': 'ios',
            'App-OS-Version': '12.2',
            'App-Version': '7.6.2',
    ...
    async def user_detail(self, user_id, filter='for_ios', req_auth=True):
        ...
        r = await self.no_auth_requests_call('GET', url, params=params, req_auth=req_auth)
    ...
    # 趋势标签 (Search - tags)
    async def trending_tags_illust(self, filt='for_ios', req_auth=True):
    ...
    async def search_illust(self, word, search_target='partial_match_for_tags',
    ...
    # 新增收藏
    async def illust_bookmark_add(self, illust_id, restrict='public', req_auth=True):
        url = '%s/v2/illust/bookmark/add' % self.hosts
        data = {
            'illust_id': illust_id,
            'restrict': restrict,
        }
        # TODO: tags mast quote like
        # 'tags=%E5%B0%BB%E7%A5%9E%E6%A7%98%20%E8%A3%B8%E8%B6%B3%20Fate%2FGO'
        # if (type(tags) == str):
        #     data['tags'] = tags
        # if (type(tags) == list):
        #     data['tags'] = " ".join([ str(tag) for tag in tags ])

        r = await self.no_auth_requests_call('POST', url, data=data, req_auth=req_auth)
        return self.parse_json(r)
    ...
    async def showcase_article(self, showcase_id):
        ...
        r = await self.no_auth_requests_call('GET', url, headers=headers, params=params, req_auth=False)
        return self.parse_json(r)

    async def parse_qs(self, next_url):
        if not next_url:
            return None
        result_qs = {}
