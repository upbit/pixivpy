    ...
    if pydantic.__version__.startswith("2."):
        ...
        class Config:
            ...
            allow_population_by_field_name = (
                True  # Allow using both snake_case and camelCase
            )
...
class WebUserWorkspace(BaseWebModel):
    ...
    workspace_image_url: Optional[str] = Field(
        default=None, alias="userWorkspaceImageUrl"
    )
...
class WebNovelInfoBase(BaseWebModel):
    ...
    cover_url: str = Field(
        validation_alias=AliasChoices("url", "coverUrl")
    ...
    zone_config: Any = Field(
        default=None, exclude=True
    )  # Exclude from model validation
...
class WebUserProfileAll(BaseWebModel):
    ...
    pickup: Optional[List[Union[WebPickupFanbox, WebPickupIllust, WebPickupNovel]]] = (
        None
    )
...
class WebListedUserIllust(BaseWebModel):
    ...
    alt: str
    titleCaptionTranslation: Optional[Any] = (
        None  # Simplified, assuming structure is similar
    )
    create_date: datetime.datetime
    update_date: datetime.datetime
    is_unlisted: bool
